...
Route::post('/admin/chuc-vu/search', [ChucVuController::class, 'searchChucVu'])->middleware('nhanVienMiddle');
...
// PHÂN QUYỀN
Route::get('/admin/phan-quyen/chi-tiet-data', [PhanQuyenController::class, 'getData'])->middleware('nhanVienMiddle');
...
Route::post('/khach-hang/update-avatar', [KhachHangController::class, 'updateAvatar']);
...
// Thêm route này vào nhóm khách hàng
...
// Admin - thống kê dashborad
Route::get('/admin/thong-ke/dashboard', [ThongKeAdminController::class, 'dashboard'])->middleware('nhanVienMiddle');

Route::get('/transaction', [TestController::class, 'GetTransaction']);
Route::get('/text', [TestController::class, 'convert']);

// ===========================================    BROADCASTING AUTH    ===============================================
// Route để authorize private channels cho Laravel Broadcasting
// Route này tương thích với Frontend đang gọi /api/broadcasting/auth
//
// Lưu ý: Tất cả user types (quan_an, khach_hang, shipper) đều dùng sanctum guard
// nhưng model khác nhau (QuanAn, KhachHang, Shipper)
Route::post('/broadcasting/auth', function (Request $request) {
    ...
    // Authenticate với sanctum guard (tất cả user types đều dùng sanctum)
    $user = Auth::guard('sanctum')->user();

    if (!$user) {
        Log::warning('Broadcasting auth: No user authenticated', [
    ...
    // QUAN TRỌNG: Set user vào request để routes/channels.php có thể sử dụng
    $request->setUserResolver(function () use ($user) {
        return $user;
    });

    // Gọi Broadcast::auth() - Laravel sẽ tự động xử lý authorization
    try {
